...
function SocialIcon({ type, url }: { type: string; url: string }) {
  ...
  return (
    <a
      ...
      href={url}
      aria-label={`@iamdipakparmar ${type.toLowerCase()}`}
      className="z-0 px-2 pt-0 pb-3 m-0 align-baseline border-0 cursor-pointer transition-transform duration-[0s]"
    >
      <svg
        enableBackground="new 0 0 24 24"
        viewBox="0 0 24 24"
        className="w-8 h-8 pointer-events-none fill-blue-500"
      >
        <title>{type.toLowerCase()}</title>
        <path d={path} />
      </svg>
    </a>
  );
