...
import type {
  CheckStatus,
  LiveAuthVerificationResponse,
  LiveCheck
} from '@/lib/message-auth-live';
...
export function buildAtpsDnsName(
  signerDomain: string,
  authorDomain: string,
  hashAlg: 'sha256' | 'none'
): string {
  const label =
    hashAlg === 'none'
      ...
      : base32Encode(
          createHash('sha256').update(signerDomain.toLowerCase()).digest()
        );
...
const DNS_PROVIDER_PATTERNS: Array<{ name: string; pattern: RegExp }> = [
  ...
  { name: 'Google Cloud DNS', pattern: /googledomains\.com\.?$/i },
  {
    ...
    pattern: /azure-dns\.(com|net|org|info)\.?$/i
  },
...
function ipv6ToNibbles(ip: string): string {
  ...
  const left = halves[0]
    ? halves[0].split(':').map((p) => p.padStart(4, '0'))
    : [];
  const right =
    halves.length > 1 && halves[1]
      ? halves[1].split(':').map((p) => p.padStart(4, '0'))
      : [];
...
export function expandSpfMacros(
  template: string,
  ip: string,
  domain: string
): string | null {
...
function normalizeTxtRecord(raw: string): string {
  const quotedParts = Array.from(raw.matchAll(/"([^"]*)"/g)).map(
    (match) => match[1]
  );
...
export function isIpv6InCidr(ip: string, cidr: string) {
  ...
  const ipBinary = expandedIp
    .map((segment) =>
      Number.parseInt(segment, 16).toString(2).padStart(16, '0')
    )
    .join('');
  const baseBinary = expandedBase
    .map((segment) =>
      Number.parseInt(segment, 16).toString(2).padStart(16, '0')
    )
    .join('');
...
export function enforceSpfLookupLimit(
  state: SpfEvaluationState
): SpfEvaluationResult | null {
  if (state.lookups > 10) {
    return {
      result: 'permerror',
...
export function enforceSpfVoidLookupLimit(
  state: SpfEvaluationState
): SpfEvaluationResult | null {
...
async function buildSpfTree(
  ...
  if (spfRecords.length > 1) {
    return {
      domain,
      record: spfRecords[0],
      error: 'multiple SPF records (permerror)',
      mechanisms: [],
      includes: [],
      redirect: null
    };
  ...
  const redirectTarget = redirectToken
    ? redirectToken.replace(/^redirect=/i, '')
    : null;

  const includes = await Promise.all(
    includeTargets.map((target) =>
      hasMacros(target)
        ? Promise.resolve<SpfTreeNode>({
            domain: target,
            record: null,
            error: 'macro: resolved at delivery time',
            mechanisms: [],
            includes: [],
            redirect: null
          })
...
async function evaluateSpf(
  ...
  const record = spfRecords[0];
  const tokens = record.trim().split(/\s+/).slice(1);
  const state: SpfEvaluationState = {
    lookups: 0,
    voidLookups: 0,
    loopDetected: false,
    unsupported: []
  ...
  for (const token of tokens) {
    ...
    if (mechanism === 'ip4') {
      const cidrStr = parsed.cidr4
        ? `${domainValue}/${parsed.cidr4}`
        : domainValue;
      if (isIpInCidr(ip, cidrStr)) {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched ip4:${cidrStr}.`,
          state
        };
      }
      continue;
    ...
    if (mechanism === 'ip6') {
      ...
      if (isIpInCidr(ip, cidrStr)) {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched ip6:${cidrStr}.`,
          state
        };
      }
      continue;
    ...
    if (mechanism === 'include') {
      ...
      const expanded = hasMacros(rawTarget)
        ? expandSpfMacros(rawTarget, ip, domain)
        : rawTarget;
      if (expanded === null) {
        return {
          result: 'permerror',
          explanation: `SPF include:${rawTarget} contains macros that require sender context (RFC 7208 s7).`,
          state
        };
      }
      ...
      if (included.result === 'pass') {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched include:${target}.`,
          state
        };
      }
    ...
    if (mechanism === 'a') {
      ...
      const host = hasMacros(rawHost)
        ? (expandSpfMacros(rawHost, ip, domain) ?? rawHost)
        : rawHost;
      ...
      const matched =
        (ipFamily === 4 &&
          aRecords.some((addr) => isIpv4InCidr(ip, `${addr}/${cidr4Len}`))) ||
        (ipFamily === 6 &&
          aaaaRecords.some((addr) => isIpv6InCidr(ip, `${addr}/${cidr6Len}`)));
      if (matched) {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched A/AAAA for ${host}.`,
          state
        };
      }
      continue;
    ...
    if (mechanism === 'mx') {
      ...
      const mxHosts = mxRecords
        .map((entry) =>
          entry.trim().split(/\s+/).slice(1).join(' ').replace(/\.$/, '')
        )
        .filter(Boolean);
      if (mxHosts.length === 0) {
        state.voidLookups += 1;
      ...
      if (matched) {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched MX host IPs for ${host}.`,
          state
        };
      }
      continue;
    ...
    if (mechanism === 'exists') {
      ...
      if (expanded === null) {
        return {
          result: 'permerror',
          explanation: `SPF exists:${rawTarget} contains macros that require sender context (RFC 7208 s7).`,
          state
        };
      }
      const target = expanded;
      ...
      if (addresses.length > 0) {
        return {
          result: qualifierToResult(qualifier),
          explanation: `Matched exists:${target}.`,
          state
        };
      }
      continue;
    ...
    if (mechanism === 'all') {
      return {
        result: qualifierToResult(qualifier),
        explanation: `Reached ${qualifier}all.`,
        state
      };
    }
  ...
  if (redirectTarget) {
    const expandedRedirect = hasMacros(redirectTarget)
      ? expandSpfMacros(redirectTarget, ip, domain)
      : redirectTarget;
    if (expandedRedirect === null) {
      return {
        result: 'permerror',
        explanation: `SPF redirect=${redirectTarget} contains macros that require sender context (RFC 7208 s7).`,
        state
      };
    }
    state.lookups += 1;
    ...
    return {
      ...
      explanation: redirected.explanation,
      state
    };
  }
...
function detectDnsProvider(nameserver: string | null) {
  ...
  return (
    DNS_PROVIDER_PATTERNS.find((provider) => provider.pattern.test(nameserver))
      ?.name ?? null
  );
}
...
export async function GET(request: Request) {
  try {
    ...
    const dmarcRecord =
      dmarcTxt.find((record) => record.toLowerCase().startsWith('v=dmarc1')) ??
      null;
    ...
    const dmarcChecks: LiveCheck[] = [
      dmarcRecord
        ...
        : toCheck(
            ...
            `No DMARC record found at _dmarc.${fromDomain}.`
          )
    ];
    ...
    if (dmarcRecord && dmarcStandard) {
      const standardLabels: Record<string, string> = {
        ...
        mixed:
          'Mixed - both RFC 7489 (pct) and RFC 9989 tags present. Consider removing pct.',
        compatible:
          'Compatible with both RFC 7489 and RFC 9989; no standard-specific tags.'
      };
      dmarcChecks.push(
        toCheck(
    ...
    if (dmarcRecord) {
      ...
      dmarcChecks.push(
        toCheck(
          effectivePolicy === 'reject' || effectivePolicy === 'quarantine'
            ? 'ok'
            : 'warning',
          'DMARC Policy',
          dmarcTags.p
            ? `Policy is ${dmarcTags.p}.`
            : 'Policy tag p= is missing; defaults to none.'
        )
      );
      ...
      if (dmarcTags.t === 'y') {
        ...
        dmarcChecks.push(
          toCheck(
            'warning',
            'DMARC Test Mode',
            `Test mode (t=y) is active. Policy ${effectivePolicy} is applied as ${downgraded}.`
          )
        );
      }
      ...
      if (dmarcTags.sp) {
        dmarcChecks.push(
          toCheck(
            ...
            `sp=${dmarcTags.sp} applies to existing subdomains.`
          )
        );
      }
      if (dmarcTags.np) {
        dmarcChecks.push(
          toCheck(
            'info',
            'DMARC Non-Existent Subdomain Policy',
            `np=${dmarcTags.np} applies to NXDOMAIN subdomains (RFC 9989).`
          )
        );
      }
      ...
      if (dmarcTags.pct) {
        dmarcChecks.push(
          toCheck(
            'warning',
            'DMARC pct Deprecated',
            `pct=${dmarcTags.pct} is present but removed in RFC 9989/DMARCbis. Receivers may ignore it.`
          )
        );
      }
      ...
      if (dmarcTags.fo) {
        ...
        const foLabels = dmarcTags.fo
          .split(':')
          .map((v) => foDescriptions[v] ?? v)
          .join('; ');
        dmarcChecks.push(
          toCheck(
            'info',
            'DMARC Failure Reporting',
            `fo=${dmarcTags.fo}: ${foLabels}.`
          )
        );
      }
    }

    const spfRecords = spfTxt.filter((record) =>
      record.toLowerCase().startsWith('v=spf1')
    );
      ...
      spfChecks.push(
        spfRecord
          ...
          : toCheck(
              ...
              `No SPF record found for ${context.spfDomain}.`
            )
      );
      spfChecks.push(
        toCheck(
          ...
          spfRecords.length > 1
            ...
            : 'Less than two SPF records found.'
        )
      );
      spfChecks.push(
        toCheck(
    ...
    if (spfRecord) {
      ...
      const syntaxLooksValid = spfMechanisms.every(
        (mechanism) => mechanism.description !== 'Unrecognized SPF token'
      );
      spfChecks.push(
        toCheck(
          syntaxLooksValid ? 'ok' : 'problem',
          'SPF Syntax Check',
          syntaxLooksValid
            ? 'The record is valid.'
            : 'The record contains unrecognized tokens.'
        )
      );
      spfChecks.push(
        toCheck(
          ...
          afterAll
            ? 'Additional mechanisms were found after all.'
            : 'No items were found after all.'
        )
      );
      spfChecks.push(
        toCheck(
          hasPtr ? 'warning' : 'ok',
          'SPF Type PTR Check',
          hasPtr ? 'Deprecated ptr mechanism found.' : 'No ptr mechanism found.'
        )
      );
    }
    ...
    if (spfEvaluation) {
      spfChecks.unshift(
        toCheck(
          spfEvaluation.result === 'pass'
            ? 'ok'
            : spfEvaluation.result === 'softfail'
              ? 'warning'
              : 'problem',
          'SPF Authentication',
      ...
      spfChecks.push(
        toCheck(
          ...
          'SPF Recursive Loop',
          spfEvaluation.state.loopDetected
            ? 'Recursive include/redirect loop detected.'
    ...
    const dkim = await Promise.all(
      context.dkimSignatures.map(async (signature) => {
        ...
        const record =
          records.find((value) => value.toLowerCase().includes('v=dkim1')) ??
          null;
        ...
        const checks: LiveCheck[] = [
          record
            ...
            : toCheck(
                ...
                `No DKIM record found at ${name}.`
              ),
          toCheck(
            record && tags.v?.toUpperCase() === 'DKIM1'
              ? 'ok'
              : record
                ? 'warning'
                : 'info',
            'DKIM Syntax Check',
            record
              ? 'The record shape looks valid.'
              : 'No record was available to validate.'
          ),
          toCheck(
            revoked ? 'problem' : 'ok',
            'DKIM Public Key Check',
            revoked
              ? 'Public key is empty or revoked.'
    ...
    if (context.dkimSignatures.length === 0) {
      atpsChecks.push(
        toCheck(
          'info',
          'ATPS Check',
          'No DKIM signers provided; cannot perform ATPS DNS lookup (RFC 6541).'
        )
      );
    } else {
      for (const sig of context.dkimSignatures) {
        for (const hashAlg of ['sha256', 'none'] as const) {
          ...
          const atpsRecord =
            records.find((r) => r.toLowerCase().startsWith('v=atps1')) ?? null;
          const authorized = atpsRecord !== null;
          atpsSigners.push({
            signerDomain: sig.domain,
      ...
      if (!atpsSigners.some((s) => s.authorized)) {
        atpsChecks.push(
          toCheck(
            'info',
            'ATPS Authorization',
            `No ATPS records found under _atps.${fromDomain} for provided DKIM signers.`
          )
        );
      }
      atpsChecks.push(
      ...
      spf: context.spfDomain
        ? {
            ...
            evaluation: spfEvaluation
              ? {
                  result: spfEvaluation.result,
                  explanation: spfEvaluation.explanation
                }
              : null,
    ...
    return captureAPIError(error, request, 500, {
      operation: 'message_header_live_auth'
    });
  }
}
