...
export async function POST(request: NextRequest) {
  ...
  if (versionRanges.length === 0) {
    return NextResponse.json(
      { error: 'At least one valid version range is required' },
      { status: 400 }
    );
  }

  const rangesKey = versionRanges
  ...
  try {
    ...
    const result = await fetchReleaseNotes(owner, repo, versionRanges, {
      token
    });
    ...
    return captureAPIError(error, request, 500, {
      operation: 'github_release_notes'
    });
  }
}
