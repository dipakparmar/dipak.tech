...
async function fetchWayback(domain: string): Promise<ThreatData['wayback']> {
  try {
    ...
    const [availRes, firstRes, countRes] = await Promise.allSettled([
      ...
      fetch(
        `https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(domain)}&output=json&fl=timestamp&limit=1&from=19960101`,
        { signal: controller.signal }
      ),
      fetch(
        `https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(domain)}&output=json&fl=timestamp&collapse=timestamp:8&showNumPages=true`,
        { signal: controller.signal }
      )
    ]);
    ...
    if (availRes.status === 'fulfilled' && availRes.value.ok) {
      ...
      if (ts)
        lastArchived = `${ts.slice(0, 4)}-${ts.slice(4, 6)}-${ts.slice(6, 8)}`;
    }
  ...
  try {
    ...
    if (res.ok) {
      ...
      const userAgents = new Set(
        (text.match(/^User-agent:\s*(.+)/gim) ?? []).map((l) =>
          l.replace(/^User-agent:\s*/i, '').trim()
        )
      );
      const sitemapUrls = (text.match(/^Sitemap:\s*(.+)/gim) ?? []).map((l) =>
        l.replace(/^Sitemap:\s*/i, '').trim()
      );
      Object.assign(robots, {
        found: true,
...
export async function GET(request: Request) {
  try {
    ...
    const ipsToCheck = isIP
      ? [normalized]
      : (await resolveIPs(normalized)).slice(0, 3);
    ...
    const [shodanResults, wayback, crawl] = await Promise.all([
      ...
      isIP
        ? Promise.resolve({
            ...
            sitemap: { found: false, urlCount: 0, lastModified: null }
          })
        : fetchCrawlRules(normalized)
    ]);
