...
export async function GET(request: Request) {
  try {
    ...
    if (!isIP) {
      ...
      if (dsRecords.length > 0) {
        ...
        dnssec = {
          ...
          algorithm:
            algorithms[algoNum] ??
            (isNaN(algoNum) ? null : `Algorithm ${algoNum}`)
        };
      }
    }
    ...
    if (!isIP) {
      const wildcardCheck = await dohQuery(
        `_wildcard-probe-osint._domainkey.${normalized}`,
        'TXT'
      );
      ...
      if (!dkimWildcard) {
        const dkimChecks = await Promise.allSettled(
          COMMON_DKIM_SELECTORS.map(async (sel) => {
            const records = await dohQuery(
              `${sel}._domainkey.${normalized}`,
              'TXT'
            );
    ...
    const listResults = isIP
      ? IP_BLOCKLISTS.map((l) => ({
          name: l.name,
          zone: `${reverseIP(normalized)}.${l.zone}`
        }))
      : DOMAIN_BLOCKLISTS.map((l) => ({
          name: l.name,
          zone: `${normalized}.${l.zone}`
        }));
    ...
    const blocklistResults = blocklistChecks
      .filter(
        (r): r is PromiseFulfilledResult<{ name: string; listed: boolean }> =>
          r.status === 'fulfilled'
      )
      .map((r) => r.value);
