...
export async function GET(request: NextRequest) {
  ...
  const year =
    yearRaw === currentYear || yearRaw === currentYear + 1
      ? yearRaw
      : currentYear;

  const daysRaw = parseInt(params.get('days') ?? '', 10);
  const dayOffBudget = Number.isFinite(daysRaw)
    ? Math.max(1, Math.min(60, daysRaw))
    : 15;
  ...
  const noticeByStrategy =
    base64DecodeJSON<Partial<Record<PlanStrategy, number>>>(
      params.get('notice')
    ) ?? {};
