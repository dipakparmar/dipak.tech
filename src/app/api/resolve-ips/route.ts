  ...
  try {
    ...
    if (!Array.isArray(subdomains) || subdomains.length === 0) {
      return NextResponse.json(
        { error: 'Invalid subdomains array' },
        { status: 400 }
      );
    }
...
async function resolveSubdomain(subdomain: string): Promise<ResolveResult> {
  ...
  try {
    const [aRecords, aaaaRecords, nsRecords, mxRecords, txtRecords] =
    ...
    if (aRecords.status === 'fulfilled' && aRecords.value.Answer) {
      const cnameRecords = aRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 5
      );
      if (cnameRecords.length > 0) {
        result.cname = cnameRecords[0].data;
      }
      result.ips = aRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 1
      ).map((r: DNSRecord) => r.data);
    }

    // Process AAAA records (IPv6)
    if (aaaaRecords.status === 'fulfilled' && aaaaRecords.value.Answer) {
      result.ipv6 = aaaaRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 28
      ).map((r: DNSRecord) => r.data);
    }

    // Process NS records (nameservers)
    if (nsRecords.status === 'fulfilled' && nsRecords.value.Answer) {
      result.nameservers = nsRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 2
      ).map((r: DNSRecord) => r.data);
    }

    // Process MX records (mail servers)
    if (mxRecords.status === 'fulfilled' && mxRecords.value.Answer) {
      result.mxRecords = mxRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 15
      ).map((r: DNSRecord) => r.data);
    }

    // Process TXT records
    if (txtRecords.status === 'fulfilled' && txtRecords.value.Answer) {
      result.txtRecords = txtRecords.value.Answer.filter(
        (r: DNSRecord) => r.type === 16
      ).map((r: DNSRecord) => r.data);
    }
