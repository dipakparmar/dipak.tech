...
import {
  queryDomainWhoisFallback,
  type WhoisFallbackResult
} from '@/lib/whois';
...
function shouldFallbackToWhois(
    ...
    try {
      ...
      if (!shouldFallbackToWhois(queryType, rdapError)) {
        ...
        const status =
          ...
          message.includes('No RDAP server found')
            ? 404
            : 500;
      ...
      try {
        payload = await queryDomainWhoisFallback(
          normalizedQuery,
          fallbackReason
        );
      } catch (whoisError) {
