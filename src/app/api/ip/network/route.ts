...
function extractRDAPNetworkInfo(
  rdap: Record<string, unknown>
): RDAPNetworkInfo {
  // CIDR
  let cidr = '';
  const cidr0 = rdap.cidr0_cidrs as
  ...
  if (entities) {
    for (const entity of entities) {
      ...
      const vcardArray = entity.vcardArray as
        | [string, Array<[string, Record<string, unknown>, string, string]>]
        | undefined;
...
function extractNameservers(rdap: Record<string, unknown>): string[] {
  const nameservers = rdap.nameservers as
...
async function handleIPQuery(
  ...
  const rdapType =
    queryType === 'cidr' ? (query.includes(':') ? 'ipv6' : 'ipv4') : queryType;
  ...
  if (rdapResult.status === 'fulfilled') {
    ...
    errors.push({
      source: 'rdap',
      message: rdapResult.reason?.message || 'RDAP lookup failed'
    });
  }
  ...
  const bgpState =
    bgpStateResult.status === 'fulfilled' ? bgpStateResult.value : null;
  if (bgpStateResult.status === 'rejected') {
    errors.push({
      source: 'ripestat',
      message: bgpStateResult.reason?.message || 'BGP state lookup failed'
    });
  }
  ...
  if (bgpState) {
    ...
    if (rpkiResult.status === 'fulfilled' && rpkiResult.value) {
      ...
      errors.push({
        source: 'ripestat',
        message: 'RPKI validation lookup failed'
      });
    }
...
async function handleASNQuery(query: string): Promise<NetworkIntelResponse> {
  ...
  const [overviewResult, whoisResult, prefixesResult, peersResult] =
  ...
  const prefixesData =
    prefixesResult.status === 'fulfilled' ? prefixesResult.value : null;
  const peersData =
  ...
  if (firstV4) {
    ...
    if (bgpState) {
      ...
      const rpki = await getRIPERPKIValidation(
        bgpState.origin_asn,
        bgpState.prefix
      );
...
export async function GET(request: Request) {
  ...
  if (parsed.type === 'domain') {
    return NextResponse.json(
      {
        error:
          'Resolve the domain to an IP first using /api/ip?target=example.com'
      },
      { status: 400, headers: rateLimitHeaders }
    );
  }
  ...
  try {
    ...
    return captureAPIError(error, request, 500, {
      operation: 'network-intelligence'
    });
  }
}
