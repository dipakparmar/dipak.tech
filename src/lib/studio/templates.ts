...
function named<T extends { set: (options: object) => unknown }>(
  obj: T,
  name: string
): T {
...
function pill(text: string, options: PillOptions): Group {
  ...
  group.set({
    left: options.originCenterX
      ? options.left - (label.width + padX * 2) / 2
      : options.left,
    top: options.top
  });
  return group;
...
const boldStatement: BuiltinTemplate = {
  ...
  pages: [
    async (canvas, width, height) => {
      await Promise.all([
        ensureFontLoaded(FONT_CONDENSED),
        ensureFontLoaded(FONT_GROTESK, '500')
      ]);
      ...
      canvas.add(
        named(
          blob(width * 0.92, height * 0.08, unit * 0.55, '#CCFF00', 0.13),
          'Glow'
        )
      );
      ...
      const underline = createRoughShape('scribble-underline', width * 0.34, {
        stroke: lime,
        strokeWidth: 2
      });
      underline.set({
        left: margin,
        top: height * 0.3 + unit * 0.395,
        name: 'Underline'
      } as NamedOptions);

      const body = named(
        new Textbox(
          'The best travel photos already happened in front of you. Your job is not to repaint them.',
          {
            left: margin,
            top: height * 0.62,
            width: width * 0.7,
      ...
      const arrow = createRoughShape('scribble-arrow', width * 0.13, {
        stroke: lime,
        strokeWidth: 2
      });
      arrow.set({
...
const gradientAnnouncement: BuiltinTemplate = {
  ...
  pages: [
    async (canvas, width, height) => {
      ...
      canvas.add(
        named(
          blob(width * 0.12, height * 0.12, unit * 0.5, '#FFFFFF', 0.22),
          'Blob 1'
        )
      );
      canvas.add(
        named(
          blob(width * 0.9, height * 0.85, unit * 0.55, '#FDE68A', 0.28),
          'Blob 2'
        )
      );
...
const tipsCarousel: BuiltinTemplate = {
  ...
  pages: [
    ...
    async (canvas, width, height) => {
      ...
      const ring = createRoughShape('scribble-circle', width * 0.36, {
        stroke: CAROUSEL_INK,
        strokeWidth: 2
      });
      ring.set({
        left: margin - width * 0.05,
        top: height * 0.235,
        scaleY: (ring.scaleY ?? 1) * 2.4,
        name: 'Ring'
      } as NamedOptions);
      ...
      const arrow = createRoughShape('scribble-arrow', width * 0.1, {
        stroke: CAROUSEL_ACCENT,
        strokeWidth: 2
      });
      arrow.set({
        left: width * 0.85,
    ...
    async (canvas, width, height) => {
      await Promise.all([
        ensureFontLoaded(FONT_CONDENSED),
        ensureFontLoaded(FONT_GROTESK, '500'),
        ensureFontLoaded(FONT_HANDWRITING, '700')
      ]);
      const unit = Math.min(width, height);
      ...
      canvas.add(
        named(
          blob(width * 0.85, height * 0.15, unit * 0.5, '#FF4D00', 0.25),
          'Glow'
        )
      );
...
const editorialQuote: BuiltinTemplate = {
  ...
  pages: [
    async (canvas, width, height) => {
      await Promise.all([
        ensureFontLoaded(FONT_SERIF, '600'),
        ensureFontLoaded(FONT_GROTESK, '700')
      ]);
      const unit = Math.min(width, height);
      ...
      const quote = named(
        new Textbox(
          'Somewhere between the trailhead and the summit, I stopped taking photos for likes.',
          {
            left: margin,
            top: height * 0.33,
            width: width * 0.8,
...
const blogCover: BuiltinTemplate = {
  ...
  pages: [
    async (canvas, width, height) => {
      ...
      canvas.add(
        named(
          blob(width * 0.92, height * 0.2, unit * 0.7, '#38BDF8', 0.18),
          'Glow'
        )
      );
      ...
      const underline = createRoughShape('scribble-underline', width * 0.24, {
        stroke: sky,
        strokeWidth: 2
      });
      underline.set({
