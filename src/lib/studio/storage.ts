...
function migratePages(value: { pages?: string[]; json?: string }): string[] | null {
...
export function loadTemplates(): SavedTemplate[] {
  const raw = safeRead<(SavedTemplate & { json?: string })[]>(TEMPLATES_KEY) ?? []
...
export function loadAutosave(): AutosavePayload | null {
  ...
  const activeIndex = Number.isInteger(raw.activeIndex) ? Math.min(raw.activeIndex, pages.length - 1) : 0
 ...
 */
export async function downloadImagesAsZip(filename: string, images: { name: string; dataUrl: string }[]): Promise<void> {
 ...
 */
export function fileToImageDataUrl(file: File, maxEdge = 2400): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        resolve(hasAlpha ? canvas.toDataURL("image/png") : canvas.toDataURL("image/jpeg", 0.9))
      }
