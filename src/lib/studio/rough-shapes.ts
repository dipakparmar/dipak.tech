 ...
 */
const SCRIBBLES: Record<
  string,
  {
    viewBox: [number, number];
    paths: [string, number, number][];
    filled?: boolean;
  }
> = {
  'scribble-circle': {
    ...
    paths: [
      [
        'M 32 5 C 17 4, 6 9, 5 14 C 4 21, 17 25, 32 24 C 47 24, 56 20, 55 13 C 54 7, 40 3, 26 4 C 18 5, 11 8, 8 12',
        1.4,
        0.92
      ],
  ...
  'scribble-underline': {
    ...
    paths: [
      [
        'M 1 3 C 5 1, 11 4.5, 17 2.5 S 30 5, 37 2.5 S 51 1.5, 58 4 S 73 5, 81 2.5 S 93 4.5, 99 3',
        1.6,
        0.92
      ],
      [
        'M 2 4.5 C 7 3, 14 5.5, 22 4 S 34 5.5, 42 4.5 S 54 3.5, 62 5 S 77 6, 85 4.5 S 95 5.5, 99 4.2',
        1,
        0.5
      ]
    ]
  ...
  'scribble-highlight': {
    viewBox: [100, 16],
    paths: [
      ['M 1 4 C 25 2, 50 5, 99 3 L 99 13 C 75 14, 50 12, 1 13 Z', 0, 0.55]
    ],
    filled: true
  }
};
 ...
 */
export function createRoughShape(
  kind: RoughShapeKind,
  size: number,
  ...
  const drawables = (() => {
    switch (kind) {
      ...
      case 'star': {
        ...
        return [
          generator.polygon(
            starPoints(outer, outer, outer, outer * 0.45),
            roughOptions
          )
        ];
      }
      case 'heart':
        return [
          generator.path(HEART_PATH, {
            ...roughOptions,
            strokeWidth: options.strokeWidth * 0.75
          })
        ];
      case 'arrow': {
        const h = size * 0.5;
  ...
  const paths = drawables.flatMap((drawable) =>
    generator.toPaths(drawable).map(pathInfoToFabric)
  );
