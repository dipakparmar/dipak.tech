...
export function extractDomainFromAddress(value: string | null | undefined): string | null {
  if (!value) return null
...
function parseReceivedSpf(value: string | null): { domain: string | null; clientIp: string | null } {
  if (!value) {
    return { domain: null, clientIp: null }
  }

  const explicitMailFrom = value.match(/envelope-from=([^\s;]+)/i)?.[1]
  const descriptiveMailFrom = value.match(/domain of ([^\s)]+) designates/i)?.[1]
  const clientIp = value.match(/client-ip=([^\s;]+)/i)?.[1] ?? null

  return {
    domain: normalizeSpfIdentity(explicitMailFrom ?? descriptiveMailFrom ?? null),
    clientIp,
  }
}

function parseDkimHeader(value: string): { domain: string | null; selector: string | null } {
  const domain = normalizeDomain(value.match(/(?:^|;)\s*d=([^;\s]+)/i)?.[1] ?? null)
  const selector = value.match(/(?:^|;)\s*s=([^;\s]+)/i)?.[1]?.trim().toLowerCase() ?? null
...
export function deriveLiveAuthLookupContext(
  ...
  const returnPathDomain = extractDomainFromAddress(getHeaderValue(headers, "return-path"))
  ...
  for (const result of authentication.results) {
    ...
    if (result.method.toLowerCase() === "spf") {
      spfDomain = spfDomain ?? normalizeSpfIdentity(fields["smtp.mailfrom"] ?? null)
    ...
    if (result.method.toLowerCase() === "dkim") {
      const domain = normalizeDomain(
        fields["header.d"] ??
          (fields["header.i"]?.includes("@") ? fields["header.i"].split("@").pop() ?? null : null)
      )
  ...
  const uniqueDkim = Array.from(
    new Map(dkimSignatures.map((signature) => [`${signature.selector}:${signature.domain}`, signature])).values()
  )
