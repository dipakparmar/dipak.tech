...
interface ProviderHeaderRule extends ProviderHeaderGuide {
  ...
  headerName?: string;
  valuePattern?: RegExp;
  parseValue?: (value: string) => ProviderHeaderValueToken[];
}
...
const POSTMARK_SMTP_SERVICE = 'https://postmarkapp.com/smtp-service';
...
const PROVIDER_DEFINITIONS: ProviderDefinition[] = [
  {
    ...
    note: 'These headers suggest the message passed through SendGrid infrastructure, but they do not by themselves prove the visible From address is trustworthy.',
    rules: [
      {
        exact: 'x-sg-id',
        title: 'X-SG-ID',
        ...
        howToRead:
          'Treat the value as an opaque SendGrid message ID rather than something end users can decode.',
        references: [
          {
            label: 'SendGrid reserved headers',
            url: SENDGRID_RESERVED_HEADERS_DOC
          },
      ...
      },
      {
        ...
        title: 'X-SG-EID',
        description:
          'SendGrid internal event identifier tied to analytics and tracking.',
        why: 'Helpful for matching a delivered message to SendGrid activity records.',
        howToRead:
          'Opaque token. Most useful when cross-referencing SendGrid event data.',
        references: [
          {
            label: 'SendGrid reserved headers',
            url: SENDGRID_RESERVED_HEADERS_DOC
      ...
      },
      {
        ...
        title: 'X-Message-ID',
        description:
          'Provider correlation identifier commonly used by SendGrid for event lookup.',
        why: 'Lets operators map the raw email to provider-side logs and webhook events.',
        howToRead:
          'Use it as a correlation key. Do not treat it as the same thing as RFC Message-ID.',
        references: [
          { label: 'SendGrid glossary', url: SENDGRID_X_MESSAGE_ID_DOC },
          {
            label: 'SendGrid support article',
            url: SENDGRID_X_MESSAGE_ID_SUPPORT
          }
        ]
      },
      {
        ...
        howToRead:
          'Parse it as JSON if present. Look for categories, filters, and custom arguments.',
        references: [
  ...
  },
  {
    ...
    note: 'These headers are usually more useful for campaign attribution and allowlisting than for security decisions on their own.',
    rules: [
      {
        headerName: 'x-mailer',
        ...
        description:
          'Mailchimp campaign mailer identifier, often including a campaign ID.',
        why: 'A strong indicator that the message was sent through Mailchimp marketing infrastructure.',
        howToRead:
          'Look for the MailChimp mailer string and any embedded CID-style campaign token.',
        references: [
          {
            label: 'Observed campaign header example',
            url: MAILCHIMP_X_MAILER_SOURCE
          }
        ]
      },
      {
        ...
        howToRead:
          'Treat as an account-scoped ID, often useful for allowlisting or support.',
        references: [
          {
            label: 'Observed account identifier reference',
            url: MAILCHIMP_X_MC_USER_SOURCE
          }
        ]
      },
      {
        ...
        howToRead:
          'Usually contains a reporting URL rather than a human-readable diagnostic value.',
        references: [
          {
            label: 'Observed abuse header example',
            url: MAILCHIMP_X_REPORT_ABUSE_SOURCE
          }
        ]
      }
    ]
  },
  {
    ...
    note: 'These headers are often application-defined and best used for correlation, not trust decisions.',
    rules: [
      {
        exact: 'x-mc-tags',
        title: 'X-MC-Tags',
        ...
        howToRead:
          'Usually a comma-separated list of tags such as password-reset or receipt.',
        references: [
          {
            label: 'Mailchimp Transactional tags docs',
            url: MANDRILL_TAGS_METADATA_DOC
          }
        ]
      },
      {
        ...
        title: 'X-MC-Metadata',
        description:
          'Mandrill metadata blob for application-defined key/value tracking.',
        why: 'Useful for tracing the message back to internal user or event IDs.',
        howToRead:
          'Often JSON or a compact key/value structure with app-specific identifiers.',
        references: [
          {
            label: 'Mailchimp Transactional tags docs',
            url: MANDRILL_TAGS_METADATA_DOC
  ...
  },
  {
    ...
    note: 'Postmark metadata is operationally useful, especially when debugging transactional mail flow or template usage.',
    rules: [
      {
        exact: 'x-pm-tag',
        title: 'X-PM-Tag',
        ...
        howToRead:
          'Usually a short label such as welcome-email or password-reset.',
        references: [
      ...
      },
      {
        ...
        howToRead:
          'Look for a stream name like outbound or a custom stream identifier.',
        references: [
          { label: 'Postmark SMTP docs', url: POSTMARK_SMTP_DOC },
          {
            label: 'Postmark message stream guide',
            url: POSTMARK_MESSAGE_STREAM_DOC
          }
        ]
      },
      {
        ...
        howToRead:
          'Everything after the prefix is the metadata key. The value is the metadata payload.',
        references: [
          { label: 'Postmark SMTP docs', url: POSTMARK_SMTP_DOC },
          {
            label: 'Postmark SMTP service examples',
            url: POSTMARK_SMTP_SERVICE
          }
        ]
      },
      {
        ...
        references: [
          {
            label: 'Postmark tracking opens docs',
            url: POSTMARK_TRACK_OPENS_DOC
          }
        ]
      },
      {
        ...
        howToRead:
          'Common values include HtmlAndText, HtmlOnly, TextOnly, and None.',
        references: [
          {
            label: 'Postmark tracking opens docs',
            url: POSTMARK_TRACK_OPENS_DOC
      ...
      },
      {
        ...
        howToRead:
          'Expect true when the sender asked Postmark not to replace Message-ID.',
  ...
  },
  {
    ...
    note: 'These are usually sender-controlled Mailgun features and help explain template personalization or analytics behavior.',
    rules: [
      {
        exact: 'x-mailgun-tag',
        title: 'X-Mailgun-Tag',
        ...
        howToRead:
          'Multiple instances can appear. Each value is a category label.',
      ...
      },
      {
        ...
        howToRead:
          'Usually JSON. Parse it if you need sender-provided context.',
        references: [
      ...
      },
      {
        ...
        howToRead:
          'Treat the value as the template selector configured by the sender.',
        references: [{ label: 'Mailgun SMTP docs', url: MAILGUN_SMTP_DOC }]
      },
      {
        ...
        description:
          'Mailgun control for stripping Mailgun headers from the final message.',
        why: 'Explains why some expected Mailgun headers may be absent.',
        howToRead:
          'A value like all means Mailgun-specific headers were intentionally suppressed.',
        references: [{ label: 'Mailgun SMTP docs', url: MAILGUN_SMTP_DOC }]
      }
    ]
  },
  {
    ...
    note: 'These IDs are mostly operational and are best used for provider-side tracing.',
    rules: [
      {
        exact: 'x-mailin-eid',
        title: 'X-Mailin-EID',
        ...
        references: [
          { label: 'Observed Brevo header sample', url: BREVO_HEADER_SAMPLE }
        ]
      },
      {
        ...
        howToRead:
          'Treat as an internal provider ID rather than the RFC Message-ID.',
        references: [
          {
            label: 'Reserved header discussion',
            url: BREVO_RESERVED_HEADERS_SOURCE
          }
        ]
      }
    ]
  },
  {
    ...
    note: 'SES often relies more on envelope/authentication context than on many custom X- headers.',
    rules: [
      {
        exact: 'x-ses-outgoing',
        title: 'X-SES-Outgoing',
        description:
          'Marks mail as processed by Amazon SES outbound infrastructure.',
        why: 'A good indicator that the message traversed SES.',
        howToRead: 'Presence matters more than the exact value.',
        references: [
          {
            label: 'Observed SES header discussion',
            url: SES_FEEDBACK_ID_ANNOUNCEMENT
          }
        ]
      },
      {
        ...
        references: [
          { label: 'SES event publishing docs', url: SES_EVENT_PUBLISHING_DOC }
        ]
      }
    ]
  },
  {
    ...
    note: 'These are some of the most useful headers for understanding Microsoft-side filtering and final mailbox routing decisions.',
    rules: [
      {
        ...
        parseValue: parseMicrosoftForefrontAntispamReport,
        references: [
          {
      ...
      },
      {
        ...
        references: [
          {
            label: 'Microsoft antispam stamps',
            url: MICROSOFT_ANTISPAM_STAMPS_DOC
          }
        ]
      },
      {
      ...
      },
      {
        ...
        description:
          'Microsoft classification for the broad message traffic type.',
      ...
      },
      {
        ...
        description:
          'Microsoft transport diagnostic chain for message classification.',
        why: 'Useful when reconstructing which Microsoft hosts handled the message and what traffic type they assigned.',
        howToRead:
          'Read it as host-to-classification pairs rather than an end-user facing status.',
        references: [
          {
            label: 'Microsoft anti-spam headers docs',
            url: MICROSOFT_HEADERS_DOC
      ...
      },
      {
        ...
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
          }
        ]
      },
      {
      ...
      },
      {
        ...
        howToRead:
          'Common values are 1 or 0. Treat it as an internal validation signal, not a full auth verdict.',
        references: [
          {
            label: 'Microsoft anti-spam headers docs',
            url: MICROSOFT_HEADERS_DOC
      ...
      },
      {
        ...
        description:
          'Microsoft relay classification flag used during anti-spam processing.',
        why: 'Adds context about whether Exchange treated the message as relayed in the filtering pipeline.',
        howToRead:
          'Interpret it as an internal transport flag rather than a user-facing spam score.',
        references: [
          {
            label: 'Microsoft anti-spam headers docs',
            url: MICROSOFT_HEADERS_DOC
      ...
      },
      {
        ...
        description:
          'Count of Microsoft anti-spam metadata chunks attached to the message.',
        why: 'Explains whether additional chunked anti-spam data headers should be present.',
        howToRead:
          'The value is the number of companion X-MS-Exchange-Antispam-MessageData-* headers.',
        references: [
          {
            label: 'Microsoft anti-spam headers docs',
            url: MICROSOFT_HEADERS_DOC
      ...
      },
      {
        ...
        howToRead:
          'Treat the value as opaque provider metadata unless you are working with Microsoft support tooling.',
        references: [
          {
            label: 'Microsoft anti-spam headers docs',
            url: MICROSOFT_HEADERS_DOC
      ...
      },
      {
        ...
        howToRead:
          'Use it as tenant context, not as a standalone trust signal.',
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
      ...
      },
      {
        ...
        description:
          'Microsoft cross-tenant authentication role for the message flow.',
        why: 'Explains whether Microsoft treated the sender as Internal, Anonymous, or another auth class.',
        howToRead:
          'Focus on the role name to understand transport trust context.',
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
      ...
      },
      {
        ...
        description:
          'Microsoft host that supplied cross-tenant authentication context.',
      ...
      },
      {
        ...
        howToRead:
          'Compare with Received headers to understand where latency was introduced.',
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
      ...
      },
      {
        ...
        description:
          'Mailbox type involved in Microsoft cross-tenant transport.',
        why: 'Useful for distinguishing hosted mailbox flows from other tenant mailbox types.',
        howToRead:
          'Treat it as mailbox-class metadata rather than a delivery result.',
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
      ...
      },
      {
        ...
        description:
          'Microsoft-stamped user principal context for the sending identity.',
        why: 'Can help correlate the message to a tenant identity, even though the value may be obfuscated or encoded.',
        howToRead:
          'Treat the value as identity correlation data, not a human-readable username.',
        references: [
          {
            label: 'Microsoft message trace docs',
            url: MICROSOFT_MESSAGE_TRACE_DOC
      ...
      },
      {
        ...
        description:
          'Transport server that stamped Microsoft cross-tenant headers.',
  ...
  },
  {
    ...
    note: 'These headers usually describe Zoho as the receiving system, not the original sender. They are useful for delivery analysis and authentication review.',
    rules: [
      {
        ...
        howToRead:
          'Values such as pass indicate Zoho verified DKIM successfully for the message.',
        references: [
      ...
      },
      {
        ...
        description:
          'Zoho Mail ARC seal over the authentication chain it observed.',
        why: 'Shows that Zoho acted as an ARC participant and sealed the prior authentication state.',
        howToRead:
          'The d=zohomail.com tag identifies Zoho as the ARC sealer; cv shows whether the prior chain validated.',
        references: [
          { label: 'Zoho Mail header options', url: ZOHO_MAIL_HEADERS_DOC }
        ]
      },
      ...
      },
      {
        ...
        description:
          'Zoho Mail’s sealed authentication verdicts for SPF, DKIM, DMARC, and prior ARC.',
        ...
        howToRead:
          'Focus on the auth method results and the mx.zohomail.com stamping host.',
        references: [
          { label: 'Zoho Mail header options', url: ZOHO_MAIL_HEADERS_DOC }
        ]
      },
      {
        ...
        description:
          'Zoho Mail authentication summary for the received message.',
        why: 'Shows what Zoho concluded about SPF, DKIM, DMARC, and ARC when it accepted the message.',
        howToRead:
          'Read each auth method result separately; do not confuse this with the sender’s own claims.',
        references: [
          { label: 'Zoho Mail header options', url: ZOHO_MAIL_HEADERS_DOC }
        ]
      }
  ...
  },
  {
    ...
    note: 'These headers help attribute a message to a specific HubSpot account or campaign.',
    rules: [
      {
        ...
        references: [
          {
            label: 'HubSpot email sending guide',
            url: HUBSPOT_EMAIL_SENDING_DOC
          }
        ]
      },
      {
  ...
  },
  {
    ...
    note: 'Only a subset of these headers is officially documented by Salesforce. Treat undocumented X-SFDC headers as helpful signals, not stable API fields.',
    rules: [
      {
        exact: 'x-sfdc-lk',
        title: 'X-SFDC-LK',
        ...
        howToRead:
          'Expect a Salesforce org ID such as 00D.... Compare it against the org you expect.',
        references: [
          {
            label: 'Salesforce system email help',
            url: SALESFORCE_SYSTEM_EMAIL_HELP
          },
          { label: 'Salesforce Help', url: SALESFORCE_ORG_ID_HELP },
          {
            label: 'Salesforce developer docs',
            url: SALESFORCE_EMAIL_HEADER_DOC
          }
        ]
      },
      {
        ...
        howToRead:
          'Expect a Salesforce user ID. Most useful during auditing or incident response.',
        references: [
          {
            label: 'Salesforce system email help',
            url: SALESFORCE_SYSTEM_EMAIL_HELP
      ...
      },
      {
        ...
        howToRead:
          'Expect a Salesforce record ID. Use the object prefix to infer the record type if needed.',
        references: [
          {
            label: 'Salesforce system email help',
            url: SALESFORCE_SYSTEM_EMAIL_HELP
          ...
          {
            label: 'Observed header example',
            url: SALESFORCE_OBSERVED_ENTITY_SOURCE
          }
        ]
      },
      {
        ...
        howToRead:
          'Opaque identifier intended for correlation, not user interpretation.',
        references: [
          {
            label: 'Observed header example',
            url: SALESFORCE_OBSERVED_ENTITY_SOURCE
      ...
      },
      {
        ...
        howToRead:
          'Usually contains the org ID used by the relay layer for validation.',
        references: [
          { label: 'Community relay example', url: SALESFORCE_RELAY_SOURCE }
        ]
      },
      {
        exact: 'x-sfdc-app',
        ...
        references: [
          { label: 'Trailblazer community example', url: SALESFORCE_APP_SOURCE }
        ]
      },
      {
        ...
        howToRead:
          'Look for category names such as workflowActionAlert or other internal Salesforce labels.',
        references: [
          {
            label: 'Salesforce system email help',
            url: SALESFORCE_SYSTEM_EMAIL_HELP
          ...
          {
            label: 'Trailblazer community example',
            url: SALESFORCE_EMAIL_CATEGORY_SOURCE
          }
        ]
      },
      {
      ...
      },
      {
        ...
        howToRead:
          'Often shows whether the message was sent via internal Salesforce infrastructure.',
        references: [
          { label: 'Community relay example', url: SALESFORCE_RELAY_SOURCE }
        ]
      },
      ...
      },
      {
        ...
        howToRead:
          'Expect a cipher suite such as ECDHE-RSA-AES256-GCM-SHA384, or None if TLS was not used.',
        references: [
          { label: 'Salesforce TLS help', url: SALESFORCE_TLS_HELP },
          {
            label: 'Salesforce developer docs',
      ...
      },
      {
        ...
        description:
          'Signals that Salesforce Email Relay should not re-relay this message.',
        why: 'Helps explain relay behavior when Salesforce mail traverses a relay configuration.',
        howToRead:
          'A value like 1 usually indicates relay bypass was requested.',
        references: [
          {
            label: 'Observed header example',
            url: SALESFORCE_OBSERVED_ENTITY_SOURCE
      ...
      },
      {
        ...
        references: [
          {
            label: 'Community verification thread',
            url: SALESFORCE_TLS_VERIFIED_SOURCE
          }
        ]
      },
      {
        ...
        title: 'X-SFDC-X-*',
        description:
          'Custom Visualforce or Apex email header added by Salesforce template logic.',
        why: 'These headers can carry application-specific business context that is useful during investigations.',
        howToRead:
          'Everything after X-SFDC-X- is the custom header name that Salesforce derived from template configuration.',
        references: [
          {
            label: 'Salesforce developer docs',
            url: SALESFORCE_EMAIL_HEADER_DOC
      ...
      },
      {
        ...
        description:
          'Inbound-only Salesforce recipient marker used by Email-to-Case or Email-to-Apex flows.',
        why: 'Can explain how Salesforce routed an inbound message, but it is undocumented and unsupported.',
        howToRead:
          'Treat as an internal inbound-routing field whose format may change without notice.',
        references: [
          { label: 'Salesforce Help caveat', url: SALESFORCE_INBOUND_CAVEAT }
        ]
      },
      {
        exact: 'x-sender',
        title: 'X-Sender',
        description:
          'Standard header Salesforce often sets to postmaster@salesforce.com.',
        why: 'Useful as a weak platform clue when seen alongside stronger X-SFDC headers.',
        howToRead:
          'Do not rely on this alone for attribution; pair it with X-SFDC headers.',
        references: [
          {
            label: 'Salesforce developer docs',
            url: SALESFORCE_EMAIL_HEADER_DOC
      ...
      },
      {
        ...
        description:
          'Sender header commonly used by Salesforce org-wide email delivery.',
        why: 'Useful as supporting evidence that Salesforce infrastructure sent the message.',
        howToRead:
          'Use it as a secondary clue only; the stronger identifiers are X-SFDC-LK and related headers.',
        references: [
          {
            label: 'Salesforce developer docs',
            url: SALESFORCE_EMAIL_HEADER_DOC
  ...
  },
  {
    ...
    note: 'SFMC often relies on encoded return paths for bounce tracking; that context is helpful even when proprietary headers are sparse.',
    rules: [
      {
        exact: 'x-sfmc-stack',
        title: 'X-SFMC-Stack',
        ...
        references: [
          { label: 'Header walkthrough example', url: SFMC_STACK_SOURCE }
        ]
      },
      {
        headerName: 'return-path',
        valuePattern:
          /<[^>]+@\s*bounce\.[^>]+>|^[^@\s]+\.[^@\s]+\.[^@\s]+@bounce\./i,
        title: 'Return-Path with SFMC bounce encoding',
        description:
          'Marketing Cloud bounce address that commonly encodes Subscriber ID, Job ID, and MID.',
...
export function getProviderHeaderValueTokens(
  headerName: string,
  headerValue: string
): ProviderHeaderValueToken[] {
  const header: HeaderEntry = {
  ...
  for (const provider of PROVIDER_DEFINITIONS) {
    const rule = getMatchingRule(provider, header);
    if (!rule?.parseValue) continue;

    return rule.parseValue(headerValue);
  }
