...
type InMsg = { type: 'separate'; left: Float32Array; right: Float32Array; modelUrl?: string }
...
self.onmessage = async (e: MessageEvent<InMsg>) => {
  ...
  try {
    ...
    const proc = new DemucsProcessor({
      ...
      onLog: (category: string, message: string) => post({ type: 'log', category, message })
    })
    ...
    const stems: Stem[] = (['vocals', 'drums', 'bass', 'other'] as const).map((name) => ({
      name,
