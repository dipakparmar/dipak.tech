...
import { optimizeDays, optimizeDaysAsync, STRATEGY_NOTICE_DEFAULTS } from "./optimizer"
...
describe("STRATEGY_NOTICE_DEFAULTS", () => {
  test("has an entry for every strategy", () => {
    const strategies = ["balanced", "longWeekends", "miniBreaks", "weekLongBreaks", "extendedVacations"]
    for (const s of strategies) {
      expect(STRATEGY_NOTICE_DEFAULTS[s as keyof typeof STRATEGY_NOTICE_DEFAULTS]).toBeNumber()
    }
  })
...
describe("zero budget", () => {
  ...
  test("returns no breaks when effective budget is zero after taken days consume it all", () => {
    ...
    const result = optimizeDays(inputs({
      dayOffBudget: 3,
      takenDaysOff: [
...
describe("day list", () => {
  ...
  test("weekends are correctly labelled", () => {
    const result = optimizeDays(inputs({ dayOffBudget: 0, referenceDate: "2027-01-01" }))
...
describe("taken days annotation", () => {
  ...
  test("marks a date-range of taken days", () => {
    const result = optimizeDays(inputs({
      dayOffBudget: 10,
      takenDaysOff: [{ name: "Holiday", startDate: "2027-02-01", endDate: "2027-02-03" }],
    }))
  ...
  test("fractional PTO cost for half-day taken entry", () => {
    const result = optimizeDays(inputs({
      dayOffBudget: 5,
      takenDaysOff: [{ name: "Half day", date: "2027-03-01", startTime: "09:00", endTime: "13:00" }],
    }))
  ...
  test("deducts taken PTO from budget before optimizing", () => {
    const withTaken = optimizeDays(inputs({
      dayOffBudget: 5,
...
describe("budget enforcement", () => {
  ...
  test("never plans more PTO than budget after taken days", () => {
    ...
    expect(result.stats.totalDayOffs + result.stats.totalTakenDays).toBeLessThanOrEqual(10)
  })

  test("spends the full budget when opportunities exist", () => {
    // Full year, no holidays, 10 PTO - should spend all of them
    const result = optimizeDays(inputs({ dayOffBudget: 10, referenceDate: "2027-01-01" }))
...
describe("break structure", () => {
  test("all breaks have at least one day", () => {
    const result = optimizeDays(inputs({ dayOffBudget: 10, holidays: MONDAY_HOLIDAYS_2027 }))
  ...
  test("category counters sum to totalDays", () => {
    const result = optimizeDays(inputs({ dayOffBudget: 10, holidays: MONDAY_HOLIDAYS_2027 }))
    for (const b of result.breaks) {
      const sum = b.dayOffCount + b.holidayCount + b.weekendCount + b.customDayCount + b.alreadyTakenCount
  ...
  test("breaks are sorted chronologically", () => {
    ...
    for (let i = 1; i < result.breaks.length; i++) {
      expect(result.breaks[i].startDate > result.breaks[i - 1].startDate).toBe(true)
    }
  })

  test("no two breaks overlap", () => {
    const result = optimizeDays(inputs({ dayOffBudget: 15, holidays: MONDAY_HOLIDAYS_2027 }))
    for (let i = 1; i < result.breaks.length; i++) {
...
describe("holiday efficiency", () => {
  test("a Monday public holiday produces a break containing that holiday", () => {
    ...
    const breakWithHoliday = result.breaks.find(b => b.days.some(d => d.date === "2027-01-18"))
  ...
  test("a Friday public holiday produces a break containing that holiday", () => {
    ...
    const breakWithHoliday = result.breaks.find(b => b.days.some(d => d.date === "2027-12-24"))
    expect(breakWithHoliday).toBeDefined()
  })

  test("efficiency is higher when holidays are present vs absent", () => {
    const withHol = optimizeDays(inputs({
      dayOffBudget: 5,
    ...
    const effWithout = withoutHol.stats.totalDaysOff / withoutHol.stats.totalDayOffs
...
describe("expandToAdjacentFixedOff", () => {
  ...
  test("adjacent holiday is absorbed into the break", () => {
    ...
    const b = result.breaks.find(b => b.days.some(d => d.date === "2027-01-18"))
    if (b) {
...
describe("extendedVacations strategy", () => {
  test("produces fewer but longer breaks", () => {
    const extended = optimizeDays(inputs({
      dayOffBudget: 15,
      strategy: "extendedVacations",
      minNoticeDays: 0,
    }))
    const balanced = optimizeDays(inputs({
      dayOffBudget: 15,
      strategy: "balanced",
      minNoticeDays: 0,
    }))
...
describe("notice window", () => {
  test("breaks start on or after referenceDate + minNoticeDays when possible", () => {
    ...
    const result = optimizeDays(inputs({ referenceDate, minNoticeDays, dayOffBudget: 10 }))
    for (const b of result.breaks) {
...
describe("stats", () => {
  test("totalDaysOff equals sum of break totalDays", () => {
    const result = optimizeDays(inputs({ dayOffBudget: 10, holidays: MONDAY_HOLIDAYS_2027 }))
    const sumFromBreaks = result.breaks.reduce((acc, b) => acc + b.totalDays, 0)
  ...
  test("totalTakenDays reflects fractional PTO cost", () => {
    const result = optimizeDays(inputs({
      dayOffBudget: 10,
      takenDaysOff: [
        { name: "Half day", date: "2027-02-01", startTime: "09:00", endTime: "13:00" },
...
describe("alreadyTakenCount", () => {
  test("already-taken working days inside a break are counted in alreadyTakenCount", () => {
    ...
    const blockWithTaken = result.breaks.find(b => b.days.some(d => d.date === "2027-06-28" && d.isAlreadyTaken))
    if (blockWithTaken) {
...
describe("BC Canada regression", () => {
  ...
  test("does not produce a 10-day continuous block from Jul 3 to Jul 12", () => {
    ...
    const offender = result.breaks.find(b => b.startDate === "2026-07-03" && b.endDate === "2026-07-12")
...
describe("determinism", () => {
  test("same inputs always produce the same output", () => {
    ...
    expect(r1.breaks.map(b => b.startDate)).toEqual(r2.breaks.map(b => b.startDate))
...
describe("optimizeDaysAsync", () => {
  test("resolves to same result as optimizeDays", async () => {
    ...
    expect(async_.breaks.map(b => b.startDate)).toEqual(sync.breaks.map(b => b.startDate))
  })
...
describe("holiday on weekend categorization", () => {
  test("a holiday that falls on a Saturday counts under holidayCount, not weekendCount", () => {
    ...
    if (day?.inOffBlock) {
      const b = result.breaks.find(br => br.days.some(d => d.date === "2027-01-02"))
...
describe("unknown strategy fallback", () => {
  test("an unrecognised strategy string behaves like balanced", () => {
    const withBadStrategy = optimizeDays(inputs({
      dayOffBudget: 10,
      strategy: "nonexistent" as unknown as import("./types").PlanStrategy,
    }))
    const withBalanced = optimizeDays(inputs({
      dayOffBudget: 10,
      strategy: "balanced",
    }))
    // Both should produce the same plan since fallback === balanced
    expect(withBadStrategy.stats.totalDayOffs).toBe(withBalanced.stats.totalDayOffs)
    expect(withBadStrategy.breaks.map(b => b.startDate)).toEqual(withBalanced.breaks.map(b => b.startDate))
  })
})
...
describe("inverted date range guards", () => {
  ...
  test("taken day range with endDate before startDate is ignored", () => {
    const result = optimizeDays(inputs({
      dayOffBudget: 5,
      takenDaysOff: [{ name: "Bad range", startDate: "2027-05-10", endDate: "2027-05-01" }],
    }))
...
describe("strategy spacing", () => {
  test("longWeekends breaks are at least 7 days apart (end to next start)", () => {
    ...
    for (let i = 1; i < result.breaks.length; i++) {
      ...
      const gap = Math.round((currStart.getTime() - prevEnd.getTime()) / 86400000) - 1
