...
function defaultSummary(br: OffBlock): string {
  const parts = [
    `Time off (${br.totalDays} day${br.totalDays === 1 ? '' : 's'})`
  ];
...
export function renderEventText(
  ...
  return {
    summary: titleTemplate?.trim()
      ? applyTemplate(titleTemplate, tokens)
      : defaultSummary(br),
...
function breakToVEVENT(
  ...
  const { summary, description } = renderEventText(
    br,
    titleTemplate,
    notesTemplate
  );
