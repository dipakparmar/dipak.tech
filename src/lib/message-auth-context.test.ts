...
import { deriveLiveAuthLookupContext, extractDomainFromAddress } from "./message-auth-context"
...
describe("message auth context", () => {
  test("extracts domains from mailbox-like values", () => {
    expect(extractDomainFromAddress("Sender <alerts@example.com>")).toBe("example.com")
...
`)
    ...
    expect(context.dkimSignatures).toEqual([{ domain: "example.com", selector: "selector1" }])
  })
    ...
    expect(parseSpfMechanisms("v=spf1 include:_spf.google.com -all")).toEqual([
      {
        ...
        description: "Matches when the included domain's SPF evaluation returns pass."
      },
      {
        prefix: "-",
        type: "all",
        ...
        description: "Always matches. It should appear at the end of the record."
      }
    ])
  })
})
