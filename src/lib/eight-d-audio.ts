...
export const PARAM_ORDER: AutomatableParam[] = [
  'rotationSpeed',
  'angle',
  'panDepth',
  'reverbAmount',
...
export function makeKeyframe(
  time: number,
  values: Omit<Keyframe, 'id' | 'time'>
): Keyframe {
  keyframeCounter += 1;
  return { id: `kf-${keyframeCounter}`, time, ...values };
}

export function keyframeFromSettings(
  time: number,
  s: EightDSettings
): Keyframe {
...
function computeCurves(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (isSpinning(s.rotationSpeed))
      phase += (Math.PI * 2 * dt) / s.rotationSpeed;
  }
  return { posX, posZ, duration: (n - 1) * dt };
}

/** Orbit angle (radians) at a song time - used by the visualiser. */
export function orbitAngleAt(
  keyframes: Keyframe[],
  direction: 'cw' | 'ccw',
  time: number
): number {
...
export function scheduleTimeline(
  ...
  const { posX, posZ, duration } = computeCurves(
    sorted,
    direction,
    songOffset,
    songDuration
  ...
  graph.panner.positionX.setValueCurveAtTime(
    posX,
    startWhen,
    Math.max(0.02, duration)
  );
  graph.panner.positionZ.setValueCurveAtTime(
    posZ,
    startWhen,
    Math.max(0.02, duration)
  );
...
function detectSections(
  ...
  const bandFor = (v: number): EnergyBand =>
    v < lowT ? 'low' : v > highT ? 'high' : 'mid';
  ...
  return merged.map((seg) => {
    ...
    return {
      start: seg.start * hop,
      end: Math.min(duration, seg.end * hop),
      level,
      band: bandFor(level)
    };
  });
 ...
 */
export function generateSmart8D(
  ...
  const targetFor = (band: EnergyBand, angle: number): AutomationValues => {
    if (band === 'high')
      return {
        rotationSpeed: clamp(baseRotation * 0.6, 2, 20),
        angle,
        panDepth: clamp(0.75 + 0.2 * intensity, 0, 1),
        reverbAmount: 0.28,
        bassBoost: 5
      };
    if (band === 'low')
      return {
        rotationSpeed: clamp(baseRotation * 1.6, 2, 20),
        angle,
        panDepth: clamp(0.4 + 0.05 * intensity, 0, 1),
        reverbAmount: 0.55,
        bassBoost: 2
      };
    return {
      ...
      angle,
      panDepth: clamp(0.6 + 0.1 * intensity, 0, 1),
      reverbAmount: 0.4,
      bassBoost: 3
    };
  };
  ...
  const pushAt = (time: number, v: AutomationValues) =>
    keyframes.push(makeKeyframe(clamp(time, 0, duration), { ...v }));
