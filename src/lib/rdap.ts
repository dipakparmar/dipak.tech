...
export async function getRDAPBootstrap(
  type: keyof typeof RDAP_BOOTSTRAP_URLS
): Promise<RDAPService> {
  const now = Date.now();
...
export function findRDAPServerForDomain(
  domain: string,
  bootstrap: RDAPService
): string | null {
...
export function findRDAPServerForIP(
  ip: string,
  bootstrap: RDAPService,
  isIPv6: boolean
): string | null {
...
export function findRDAPServerForASN(
  asn: string,
  bootstrap: RDAPService
): string | null {
...
export async function queryRDAP(
  ...
  if (!response.ok) {
    if (response.status === 404) {
      const typeLabel =
        type === 'domain' ? 'Domain' : type === 'asn' ? 'ASN' : 'IP address';
