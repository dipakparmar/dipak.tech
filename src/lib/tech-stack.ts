...
export function detectTechStack(
  ...
  if (h('x-amz-cf-id') || h('via').includes('cloudfront'))
    result.cdn.push('AWS CloudFront');
  ...
  if (html.includes('__nuxt') || html.includes('/_nuxt/'))
    result.framework.push('Nuxt');
  if (html.includes('data-astro-') || html.includes('/_astro/'))
    result.framework.push('Astro');
  ...
  if (html.includes('data-reactroot') || html.includes('react-dom')) {
    if (
      !result.framework.some((f) => ['Next.js', 'Gatsby', 'Nuxt'].includes(f))
    ) {
  ...
  if (generator) {
    ...
    else if (generator.toLowerCase().includes('ghost'))
  ...
  if (
    hasDomain(html, 'google-analytics.com') ||
    html.includes('gtag(') ||
  ...
  if (html.includes('fathom') && hasDomain(html, 'cdn.usefathom.com'))
    result.analytics.push('Fathom');
