...
function parseX509(der: Uint8Array): DecodedCert {
                  ...
                  if ((sanType & 0x1f) === 2) {
                    sans.push(
                      new TextDecoder().decode(
                        der.slice(offset, offset + sanLen)
                      )
                    );
                  }
                  offset += sanLen;
  ...
  const pkAlgorithm = signatureAlgorithm.includes('RSA')
    ? 'RSA'
    : signatureAlgorithm.includes('EC')
      ? 'EC'
      : 'Unknown';
...
export function CertDecoder() {
  ...
  const handleDecode = useCallback(async () => {
    ...
    if (!pem.includes('-----BEGIN CERTIFICATE-----')) {
      setError(
        'Invalid PEM format. Certificate must start with -----BEGIN CERTIFICATE-----'
      );
      return;
    }
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to decode certificate'
      );
      hapticTrigger('error');
    }
  }, [pem, hapticTrigger]);
  ...
  const handleFetchFromUrl = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/fetch-cert?host=${encodeURIComponent(hostname)}&port=${port}`
      );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Decode and view certificate details. Paste a PEM certificate or
            fetch directly from a website.
          </CardDescription>
        </CardHeader>
            ...
            <TabsContent value="url" className="mt-4 space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Enter a hostname to fetch its SSL/TLS certificate. Optionally
                  specify a port (default: 443).
                </p>
              </div>
