...
export function CSRGenerator({ initialValues }: CSRGeneratorProps) {
  ...
  const handleAddSan = useCallback(() => {
    ...
    if (!sanCheck.valid) {
      setSanError(
        'Invalid format. Enter a domain (example.com), email (user@example.com), or IP address (192.168.1.1)'
      );
      return;
    }
  ...
  const handleShareUrl = useCallback(async () => {
    ...
    if (formData.organizationalUnit)
      params.set('ou', formData.organizationalUnit);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!isValidDomain(formData.commonName.trim())) {
      setError(
        'Common Name must be a valid domain (e.g., example.com or *.example.com)'
      );
      return;
    }
    ...
    for (const san of sans) {
      ...
      if (!sanCheck.valid) {
        setError(
          `Invalid SAN: "${san}" is not a valid domain, email, or IP address`
        );
        return;
      }
    }
    ...
    try {
      ...
      if (keyMode === 'existing') {
        ...
        if (!parsed) {
          throw new Error(
            'Invalid PEM format. Please paste a valid private key.'
          );
        }

        if (
          parsed.type !== 'PRIVATE KEY' &&
          parsed.type !== 'RSA PRIVATE KEY' &&
          parsed.type !== 'EC PRIVATE KEY'
        ) {
        ...
        let keyAlgorithm: RsaHashedImportParams | EcKeyImportParams | null =
          null;
        ...
        for (const modulusLength of [2048, 3072, 4096]) {
          try {
            ...
            importedKey = await crypto.importKey(
              'pkcs8',
              parsed.data,
              keyAlgorithm,
              true,
        ...
        if (!importedKey) {
          throw new Error(
            "Could not import private key. Ensure it's a valid RSA or EC private key in PKCS#8 format."
          );
        }
      ...
      const csrPEM = formatPEM(
        arrayBufferToBase64(csrBer),
        'CERTIFICATE REQUEST'
      );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate a Certificate Signing Request and private key. All
            cryptographic operations happen in your browser - your private key
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFormData((prev) => ({ ...prev, keySize: value }))
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
...
-----END PRIVATE KEY-----"
                ...
                <p className="text-xs text-muted-foreground">
                  Paste your existing private key in PKCS#8 PEM format. Supports
                  RSA and ECDSA keys.
                </p>
              </div>
            </TabsContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Add domains, email addresses, or IP addresses (e.g.,
                www.example.com, user@example.com, 192.168.1.1)
              </p>
            </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleDownload(`${formData.commonName}.csr`, result.csr)
                  }
                  className="gap-1"
                >
          ...
          <Card className="border-amber-500/30">
            <CardHeader>
              ...
              <CardDescription>
                Keep this secret! Never share your private key. Store it
                securely.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
