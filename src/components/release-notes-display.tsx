      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center gap-3 py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Fetching and processing releases...
          </p>
        </CardContent>
      </Card>
    );
    ...
    body = (
      <Card>
        <CardContent className="py-10 text-center text-sm text-muted-foreground">
          No releases found. Configure the repository and version ranges above
          to get started.
        </CardContent>
      </Card>
    );
    ...
    body = (
      <div className="space-y-6">
          ...
          <CardHeader className="space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Found {releases.length} stable releases across{' '}
                  {Object.keys(versionGroups).length} version series
        ...
        {sortedGroups.map(([majorMinor, groupReleases]) => {
          const visibleGroupReleases = groupReleases.filter((release) =>
            visibleReleases.has(release.tag_name)
          );
            ...
            <Card key={majorMinor}>
              <CardHeader className="flex flex-row flex-wrap items-center gap-3">
                ...
                <CardTitle className="text-lg">
                  Version {majorMinor}.x Series
                </CardTitle>
                <Badge variant="secondary">
                  {visibleGroupReleases.length} releases
                </Badge>
              </CardHeader>
              <CardContent className="space-y-8">
                {visibleGroupReleases.map((release, index) => (
                  <div key={release.tag_name}>
                    ...
                    {index < visibleGroupReleases.length - 1 && (
