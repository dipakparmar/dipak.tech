...
function FlagBadge({ ok, label }: { ok: boolean; label: string }) {
  return ok ? (
    <span className="rounded px-1.5 py-0.5 text-xs font-semibold bg-emerald-950 text-emerald-400 border border-emerald-800">
      {label}
    </span>
  ) : (
    <span className="rounded px-1.5 py-0.5 text-xs font-semibold bg-red-950 text-red-400 border border-red-900">
      No {label}
    </span>
  );
}
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          {hasStack ? (
            <div className="space-y-2">
              {(
                ['cdn', 'framework', 'cms', 'analytics', 'server'] as const
              ).map((cat) => {
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            {identityData.securityTxt.found ? (
              <Badge className="ml-auto bg-emerald-950 text-emerald-400 border-emerald-800 text-xs">
                Found
              </Badge>
            ) : (
              <Badge
              ...
              {identityData.securityTxt.expires && (
                <div>
                  ...
                  <div className="text-xs text-foreground">
                    {identityData.securityTxt.expires.slice(0, 10)}
                  </div>
                </div>
              )}
              <div>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          <div className="space-y-2">
            {cookies.map((cookie, i) => (
              <div
                key={i}
                className="flex flex-wrap items-center justify-between gap-2 border-t border-border/40 pt-2 first:border-t-0 first:pt-0"
              >
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {cookie.sameSite ? (
                    <span
                      className={`rounded px-1.5 py-0.5 text-xs font-semibold border ${cookie.sameSite === 'None' ? 'bg-amber-950 text-amber-400 border-amber-900' : 'bg-emerald-950 text-emerald-400 border-emerald-800'}`}
                    >
                      {cookie.sameSite}
                    </span>
                  ) : (
                    <span className="rounded px-1.5 py-0.5 text-xs font-semibold bg-amber-950 text-amber-400 border border-amber-900">
                      No SameSite
                    </span>
                  )}
                </div>
