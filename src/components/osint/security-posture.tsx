    ...
    <div className="space-y-3">
          ...
          {securityData ? (
            <div>
              <div className="mb-2 flex items-center gap-2">
                <StatusBadge
                  pass={securityData.dnssec.enabled}
                  label={securityData.dnssec.enabled ? 'Enabled' : 'Disabled'}
                />
              </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <>
              ...
              {emailSecurity.dmarc.record && (
                <div className="mt-1.5 flex gap-1.5 flex-wrap">
                  {emailSecurity.dmarc.reporting && (
                    <WarnBadge label="rua reporting" />
                  )}
                  {emailSecurity.dmarc.pct < 100 && (
                    <WarnBadge label={`${emailSecurity.dmarc.pct}% policy`} />
                  )}
                </div>
              )}
            </>
              ...
              {!securityData.dkim.wildcard &&
                securityData.dkim.selectors.length > 0 && (
                  <div className="mt-2 text-xs text-muted-foreground">
                    {securityData.dkim.selectors.join(', ')}
                  </div>
                )}
            </>
          ) : (
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          {emailSecurity ? (
            <StatusBadge
              pass={emailSecurity.bimi.present}
              label={emailSecurity.bimi.present ? 'Configured' : 'Not Set'}
            />
          ) : (
            <Skeleton className="h-6 w-24" />
          )}
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-sm font-semibold">
              <span
                className={
                  securityData.blocklist.clean === securityData.blocklist.total
                    ? 'text-emerald-400'
                    : 'text-red-400'
                }
              >
              ...
              <span className="text-muted-foreground">
                /{securityData.blocklist.total} clean
              </span>
            </span>
          </div>
          <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-muted">
            <div
              ...
              style={{
                width: `${securityData.blocklist.total > 0 ? (securityData.blocklist.clean / securityData.blocklist.total) * 100 : 0}%`
              }}
            />
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 sm:grid-cols-3">
            {securityData.blocklist.results.map((r) => (
              <div key={r.name} className="flex items-center gap-1.5 text-xs">
                ...
                <span
                  className={
                    r.listed ? 'text-red-400' : 'text-muted-foreground'
                  }
                >
                  {r.name}
                </span>
