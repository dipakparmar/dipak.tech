...
export function RedirectChain({ chain }: RedirectChainProps) {
  if (chain.length === 0)
    return (
      <div className="text-sm text-muted-foreground">No redirect data</div>
    );
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        <Badge
          className={`text-xs ${chain.length === 1 ? 'bg-emerald-950 text-emerald-400 border-emerald-800' : 'bg-amber-950 text-amber-400 border-amber-900'}`}
        >
        ...
        {chain.map((hop, i) => (
          <div key={i}>
            <div className="flex items-center gap-3 rounded-lg border border-border/40 bg-muted/20 px-3 py-2">
              <Badge
                className={`shrink-0 font-mono text-xs ${statusColor(hop.status)}`}
              >
                {hop.status}
              </Badge>
