 ...
 */
export function AudioVisualizer({ analyser, getAngle, getTime, bpm, className }: AudioVisualizerProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (analyser && freqData) {
        ...
        for (let i = 0; i < smoothRef.current.length; i++) smoothRef.current[i] *= 0.9
      }
      ...
      const ring = ctx.createRadialGradient(cx, cy, baseRadius * 0.4, cx, cy, glowR)
      ring.addColorStop(0, `rgba(${accent}, ${0.12 + level * 0.25 + beatEnv * 0.15})`)
      ...
      const centerGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, centerR * 2.2)
      ...
      const orbGrad = ctx.createRadialGradient(ox, oy, 0, ox, oy, 22 + level * 20)
