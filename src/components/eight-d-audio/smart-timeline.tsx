...
export function SmartTimeline({
  ...
  const stateRef = useRef({ keyframes, duration, analysis, currentTime, activeParam, selectedId })
  useEffect(() => {
    stateRef.current = { keyframes, duration, analysis, currentTime, activeParam, selectedId }
  ...
  const yToValue = useCallback((y: number, meta: ParamMeta, h: number) => {
    ...
    return Math.max(meta.min, Math.min(meta.max, meta.min + n * (meta.max - meta.min)))
  }, [])
  ...
  useEffect(() => {
    ...
    const drawLane = (w: number, h: number, sorted: Keyframe[], meta: ParamMeta, active: boolean, selId: string | null) => {
