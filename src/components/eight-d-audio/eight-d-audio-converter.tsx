...
export function EightDAudioConverter() {
  ...
  const [activeParam, setActiveParam] = useState<AutomatableParam>("rotationSpeed")
  ...
  const getElapsed = useCallback((): number => {
    ...
    if (playbackRef.current === "playing" && ctx) {
      return Math.min(duration, offsetRef.current + (ctx.currentTime - ctxStartRef.current))
    }
    return pausedElapsedRef.current
  }, [duration])

  const getAngle = useCallback(
    () => orbitAngleAt(activeKeyframesRef.current, settingsRef.current.direction, getElapsed()),
    [getElapsed]
  )
  ...
  useEffect(() => {
    if (playbackRef.current === "playing" && audioCtxRef.current && graphRef.current) {
      const ctx = audioCtxRef.current
  ...
  const decodeFile = useCallback(
    async (file: File) => {
      setError(null)
      if (!file.type.startsWith("audio/") && !/\.(mp3|wav|ogg|m4a|aac|flac|opus|webm)$/i.test(file.name)) {
  ...
  const generateFromAnalysis = useCallback(
    (result: AnalysisResult, intensityValue: number, direction: "cw" | "ccw") => {
      setKeyframes(generateSmart8D(result, { direction, intensity: intensityValue }))
      setSelectedId(null)
    },
    []
  )
  ...
  const runSmartAnalysis = useCallback(
    (buffer: AudioBuffer, intensityValue: number, direction: "cw" | "ccw", existing?: AnalysisResult | null) => {
      if (existing) {
  ...
  const switchMode = useCallback(
    (next: Mode) => {
      setMode(next)
      if (next === "smart" && audioBuffer && keyframes.length === 0 && !isAnalyzing) {
        runSmartAnalysis(audioBuffer, intensity, settings.direction, analysis)
      }
    },
    [audioBuffer, keyframes.length, isAnalyzing, runSmartAnalysis, intensity, settings.direction, analysis]
  )
  ...
  const handleExport = useCallback(async () => {
    ...
    setError(null)
    try {
      const rendered = await render8D(audioBuffer, activeKeyframes, settings.direction, settings.volume)
  ...
  }, [audioBuffer, activeKeyframes, settings.direction, settings.volume, fileName, mode])
  ...
  const updateSetting = useCallback(<K extends keyof EightDSettings>(key: K, value: EightDSettings[K]) => {
  ...
  const updateSelectedKeyframe = useCallback(
    (param: AutomatableParam, value: number) => {
      ...
      setKeyframes((ks) => ks.map((k) => (k.id === selectedId ? { ...k, [param]: value } : k)))
    },
    [selectedId]
  )
  ...
  const moveKeyframe = useCallback(
    (id: string, time: number, value: number) => {
      setKeyframes((ks) => ks.map((k) => (k.id === id ? { ...k, time, [activeParam]: value } : k)))
    },
    [activeParam]
  )
  ...
  const addKeyframeAt = useCallback(
    (time: number) => {
      setKeyframes((ks) => {
        const at = sampleAutomation(ks.length ? ks : activeKeyframesRef.current, time)
      ...
      <div className="mx-auto w-full max-w-5xl">
        <label
          ...
          className={cn(
            ...
            dragOver ? "border-sky-500 bg-sky-500/5" : "border-border hover:border-sky-500/50 hover:bg-muted/30"
          )}
        >
          <input
            type="file"
          ...
          <div>
            <p className="text-base font-medium">
              {isDecoding ? "Decoding audio…" : "Drop an audio file or click to browse"}
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              MP3, WAV, OGG, M4A, FLAC · up to 60 MB · processed entirely in your browser
            </p>
          </div>
        </label>
        {error && (
      ...
      <div className="flex flex-col gap-6 lg:grid lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Button onClick={handlePlayPause} className="min-w-28 flex-1" size="lg">
              {playback === "playing" ? (
                <>
        ...
        <div className="min-w-0 space-y-5 rounded-2xl border bg-card p-5">
          ...
          <div className="grid grid-cols-2 gap-1 rounded-lg bg-muted p-1">
            <button
              ...
              className={cn(
                ...
                mode === "static" ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
              Static
            </button>
            <button
              ...
              className={cn(
                ...
                mode === "smart" ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
          ...
          {mode === "static" ? (
            <StaticControls settings={settings} activePreset={activePreset} onPreset={applyPreset} onChange={updateSetting} />
          ) : (
            <SmartControls
              analysis={analysis}
              ...
              onRegenerate={() => audioBuffer && runSmartAnalysis(audioBuffer, intensity, settings.direction, analysis)}
        ...
        <div className="space-y-3 rounded-2xl border bg-card p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-1 rounded-lg bg-muted p-1">
              {PARAM_ORDER.map((p) => (
                <button
                  ...
                  style={activeParam === p ? { color: `rgb(${PARAM_META[p].color})` } : undefined}
                  className={cn(
                    "flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium transition-colors",
                    activeParam === p ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                  <span
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => addKeyframeAt(currentTime)}>
          ...
          <p className="text-xs text-muted-foreground">
            Click to seek · click a point to select · drag to move · double-click to add. Colored bands show
            low / mid / high energy sections; purple ticks mark detected drops.
          </p>
        </div>
      )}
...
interface StaticControlsProps {
  ...
  onChange: <K extends keyof EightDSettings>(key: K, value: EightDSettings[K]) => void
}

function StaticControls({ settings, activePreset, onPreset, onChange }: StaticControlsProps) {
  return (
    <>
      <div>
      ...
      <div className="space-y-5">
        <ControlSlider
          ...
          max={20}
          step={0.5}
          display={settings.rotationSpeed <= 0.05 ? "Hold (no spin)" : `${settings.rotationSpeed.toFixed(1)}s / spin`}
          onChange={(v) => onChange("rotationSpeed", v)}
        />
        <ControlSlider
          icon={<Compass className="h-4 w-4" />}
          label={settings.rotationSpeed <= 0.05 ? "Direction" : "Start direction"}
        ...
        />
        <DirectionToggle direction={settings.direction} onChange={(d) => onChange("direction", d)} />
      </div>
    </>
  )
}
...
interface SmartControlsProps {
  ...
  settings: EightDSettings
  onGlobalChange: <K extends keyof EightDSettings>(key: K, value: EightDSettings[K]) => void
}
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg border bg-muted/30 p-3">
        ...
        <div className="mt-3 space-y-3">
          ...
          />
          <Button variant="outline" size="sm" className="w-full" onClick={onRegenerate} disabled={isAnalyzing}>
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {selected ? `Point @ ${formatDuration(selected.time)}` : "Selected point"}
        </p>
        {selected ? (
      ...
      <div className="space-y-4 border-t pt-4">
        ...
        />
        <DirectionToggle direction={settings.direction} onChange={(d) => onGlobalChange("direction", d)} />
      </div>
    </div>
  )
}
...
function ControlSlider({ icon, dotColor, label, value, min, max, step, display, onChange }: ControlSliderProps) {
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="flex items-center gap-1.5 text-sm font-medium">
          {icon && <span className="text-sky-500">{icon}</span>}
          {dotColor && <span className="h-2 w-2 rounded-full" style={{ backgroundColor: `rgb(${dotColor})` }} />}
          {label}
        </span>
...
function DirectionToggle({ direction, onChange }: { direction: "cw" | "ccw"; onChange: (d: "cw" | "ccw") => void }) {
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={cn(
            ...
            direction === "cw" ? "border-sky-500 bg-sky-500/10" : "border-border hover:bg-muted/50"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "flex items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm transition-colors",
            direction === "ccw" ? "border-sky-500 bg-sky-500/10" : "border-border hover:bg-muted/50"
          )}
        >
