...
export function Portfolio() {
  ...
  const getFilteredData = <T extends ItemProps>(data: T[], level: DetailLevel): T[] => {
  ...
  const filteredWorkExperience = getFilteredData(workExperienceData, detailLevel)
  ...
  return (
    <div className="min-h-screen text-muted-foreground font-light px-4 md:px-0">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="py-4">
          <h2 className="text-lg font-normal text-foreground mb-2">Resume Detail Level</h2>
          <Slider
        ...
        <header className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4">
          <Avatar className="w-20 h-20 border">
            <AvatarImage src={personalData.avatarImage} alt={personalData.name} />
            <AvatarFallback className="bg-muted-foreground text-background"> {personalData.name.split(" ").map((n) => n.charAt(0)).join("")} </AvatarFallback>
          </Avatar>
          <div className="text-center md:text-left">
            <h1 className="text-xl font-normal text-foreground dark:text-white">{personalData.name}</h1>
            <ReactMarkdown className="text-sm text-muted-foreground">{`${personalData.title} in ${personalData.location}`}</ReactMarkdown>
        ...
        <section>
          <h2 className="text-lg font-normal text-foreground mb-4">Work Experience</h2>
        ...
        <section>
          <h2 className="text-lg font-normal text-foreground mb-4">Side Projects</h2>
          <div className="space-y-6">
        ...
        <section>
          <h2 className="text-lg font-normal text-foreground mb-4">Education</h2>
          <div className="space-y-6">
        ...
        <section>
          ...
          <div className="space-y-2">
            {contactData.map((item, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-2 md:gap-4">
                <div className="text-sm text-muted-foreground rounded col-span-full md:col-span-1 text-left md:text-left md:p-1">
                  {item.name}
                </div>
                <div className="col-span-full md:col-span-1">
                  <Link href={item.link} className="flex items-center space-x-1 text-sm text-foreground">
                    <span className="hover:underline hover:underline-offset-[3px] hover:decoration-[1px] transition duration-300">{item.username}</span>
...
const Item = ({ date, title, titlelink, organization, location, description, clients, images }: ItemProps) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-2 md:gap-4">
      ...
      <div className="space-y-2 col-span-full md:col-span-1">
        {titlelink ? (
          <Link href={titlelink} className="flex items-center space-x-1 text-base font-normal text-foreground dark:text-white">
            <ReactMarkdown className="hover:underline hover:underline-offset-[3px] hover:decoration-[1px] transition duration-300 inline">{`${title} ${organization ? `at ${organization}` : ''}` }</ReactMarkdown>
            <ArrowTopRightIcon className="w-4 h-4 font-bold" />
          </Link>
        ) : (
          <ReactMarkdown className="text-base font-normal text-foreground dark:text-white">
            {`${title} ${organization ? `at ${organization}` : ''}`}
          </ReactMarkdown>
        )}
        {location && <ReactMarkdown className="text-sm text-muted-foreground">{location}</ReactMarkdown>}
        {description && (
          <ReactMarkdown 
            className="text-sm text-muted-foreground [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:list-decimal [&>ol]:pl-5 [&>ul>li]:mt-2 [&>ol>li]:mt-2"
          >
            {description}
          </ReactMarkdown>
        )}
        {clients && <ReactMarkdown className="text-sm text-muted-foreground">{clients}</ReactMarkdown>}
        {images && (
          <div className="flex space-x-2 mt-2 overflow-x-auto">
            {images.map((src, index) => (
              <Card key={index} className="w-24 h-16 overflow-hidden flex-shrink-0">
                <CardContent className="p-0">
                  <Image src={src} alt={`${title} ${index + 1}`} width={96} height={64} className="object-cover" />
                </CardContent>
              </Card>
            ))}
          </div>
...
const projectData: ItemProps[] = [
  {
    ...
    description: "A GraphQL API for BC Transit bus schedules and real-time data.",
  },
]
...
const contactData: ContactItem[] = [
  { name: "Links", username: "dipak.bio/links", link: "https://dipak.bio/links" },
  { name: "Github", username: "dipakparmar", link: "https://github.com/dipakparmar" },
  { name: "LinkedIn", username: "iamdipakparmar", link: "https://linkedin.com/in/iamdipakparmar" },
  { name: "Instagram", username: "iamdipakparmar", link: "https://instagram.com/iamdipakparmar" },
  { name: "X", username: "iamdipakparmar", link: "https://x.com/iamdipakparmar" },
]
