...
export function LaneDiagram({ lanes, ariaLabel, className }: LaneDiagramProps) {
  ...
  const height = HEADER_HEIGHT + maxNodes * STEP - GAP + captionRows + LANE_PADDING * 2;

  return (
    <svg
      ref={ref}
      viewBox={`0 0 ${width} ${height}`}
      className={`flow-diagram w-full max-w-2xl mx-auto my-6 ${className ?? ''}`}
      role="img"
        ...
        return (
          <g key={lane.title}>
            <rect
              ...
              strokeWidth="1"
            />
            <text x={centerX} y={LANE_PADDING + 24} textAnchor="middle" fontSize="14" fontWeight="600" className="fill-foreground">
              {lane.title}
            </text>
            {lane.subtitle && (
              <text x={centerX} y={LANE_PADDING + 40} textAnchor="middle" fontSize="10.5" className="fill-muted-foreground">
                {lane.subtitle}
              </text>
            )}
              ...
              return (
                <g key={node.label}>
                  ...
                  >
                    ...
                    <text x={centerX} y={y + (node.sublabel ? 23 : 32)} textAnchor="middle" fontSize="12.5" fontWeight="600" className="fill-foreground">
                      {node.label}
                    </text>
                    {node.sublabel && (
                      <text x={centerX} y={y + 40} textAnchor="middle" fontSize="10" className="fill-muted-foreground">
                        {node.sublabel}
                      </text>
                    )}
                  </motion.g>
                  ...
                  {lines.map((line, li) => {
                    ...
                    // instead of drawing through the text, which is otherwise hard to read.
                    const labelGap = 7;
                    ...
                    return (
                      <g key={li} data-color={line.color}>
                        ...
                        {line.label && (
                          <text x={x} y={labelY} textAnchor="middle" fontSize="9.5" className="fill-muted-foreground">
                            {line.label}
                          </text>
                        )}
                      </g>
                  ...
                  {node.caption && (
                    <text x={centerX} y={y + NODE_HEIGHT + 16} textAnchor="middle" fontSize="9.5" className="fill-muted-foreground">
                      {node.caption}
                    </text>
                  )}
                </g>
