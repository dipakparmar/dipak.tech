...
function hasLookupContext(context: LiveAuthLookupContext) {
  return Boolean(
    context.fromDomain || context.spfDomain || context.dkimSignatures.length > 0
  );
}
...
function buildQuery(context: LiveAuthLookupContext) {
  ...
  if (context.returnPathDomain)
    params.set('returnPath', context.returnPathDomain);
...
function CheckList({ checks }: { checks: LiveCheck[] }) {
  return (
...
const DOMAIN_MECHANISMS = new Set([
  'include',
  'a',
  'mx',
  'exists',
  ...
  return (
    <>
      ...
      >
        ...
        <td
          className={`py-2 font-mono font-semibold ${QUALIFIER_COLORS[m.prefix || '+'] ?? ''}`}
    ...
    async function run() {
      ...
      try {
        const response = await fetch(
          `/api/message-header-auth?${buildQuery(context)}`,
          {
        ...
        if (!response.ok) {
          throw new Error(
            `Live DNS lookup failed with status ${response.status}`
          );
        }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <Alert>
          <AlertDescription>
            This check runs server-side and will query live DNS using derived
            values from your pasted message. Raw headers stay local unless you
            choose to proceed.
          </AlertDescription>
        </Alert>
          ...
          <div className="space-y-4 rounded-xl border bg-muted/20 p-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                We will derive a small set of values from the message and send
                only those values to the server so it can look up current DNS
                records.
              </p>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {context.spfDomain &&
                context.spfDomain !== context.returnPathDomain && (
                  <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Use this only if you are comfortable sending these derived
                values for live DNS verification.
              </p>
            </div>
          </div>
        )}
        ...
        {approved && data && !pending && (
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              {data.context.fromDomain && (
                <DomainBadge domain={data.context.fromDomain} />
              )}
              ...
              {!data.dnsProvider.name && data.dnsProvider.nameserver && (
                <span>
                  ...
                  <DomainLink domain={data.dnsProvider.nameserver} />
                </span>
              )}
            </div>
            ...
            {data.dmarc && (
              <section className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  {data.dmarc.tags.p && (
                    <Badge variant="outline">p={data.dmarc.tags.p}</Badge>
                  )}
                  {data.dmarc.tags.sp && (
                    <Badge variant="outline">sp={data.dmarc.tags.sp}</Badge>
                  )}
                  {data.dmarc.tags.adkim && (
                    <Badge variant="outline">
                      adkim={data.dmarc.tags.adkim}
                    </Badge>
                  )}
                  {data.dmarc.tags.aspf && (
                    <Badge variant="outline">aspf={data.dmarc.tags.aspf}</Badge>
                  )}
                </div>
                {data.dmarc.record && (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {data.spf.evaluation && (
                    <Badge variant="secondary">
                      {data.spf.evaluation.result.toUpperCase()}
                    </Badge>
                  )}
                </div>
                {data.spf.tree && (
            ...
            {data.dkim.length > 0 && (
              <section className="space-y-3">
                ...
                {data.dkim.map((entry) => (
                  <div
                    key={`${entry.selector}@${entry.domain}`}
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {entry.tags.v && (
                        <Badge variant="secondary">{entry.tags.v}</Badge>
                      )}
                      {entry.tags.k && (
                        <Badge variant="secondary">k={entry.tags.k}</Badge>
                      )}
                    </div>
                    {entry.record && (
                      <div className="rounded-md bg-muted/50 p-2">
