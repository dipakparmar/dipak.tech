...
import type { DrawMode, StudioApi, TextKind } from "@/components/studio/use-studio"
...
const TEXT_BUTTONS: { kind: TextKind; label: string; family: string; className: string }[] = [
  { kind: "heading", label: "Big Headline", family: FONT_DISPLAY, className: "text-2xl" },
  { kind: "subheading", label: "Subheading", family: FONT_BODY, className: "text-base font-semibold" },
  { kind: "body", label: "A little body text", family: FONT_BODY, className: "text-sm" },
  { kind: "handwritten", label: "a handwritten note", family: FONT_HANDWRITING, className: "text-2xl" },
  { kind: "marker", label: "Marker note", family: FONT_MARKER, className: "text-base" },
]
...
export function ToolRail({ studio, onToggleLayers }: { studio: StudioApi; onToggleLayers: () => void }) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [shapeColor, setShapeColor] = useState("#FFE066")
  const [photoBusy, setPhotoBusy] = useState(false)

  const selectedImage = studio.selected.length === 1 && studio.selected[0].isType("image")

  return (
    <div className="flex shrink-0 flex-col items-center gap-1 border-r bg-background/60 p-1.5">
      <RailButton label="Select (esc)" active={studio.drawMode === "off"} onClick={() => studio.setDrawMode("off")}>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {ROUGH_SHAPE_LABELS.map((shape) => (
              <button
                key={shape.kind}
                type="button"
                onClick={() => studio.addShape(shape.kind, { stroke: shapeColor, strokeWidth: 3 })}
              ...
              >
                ...
                <span className="text-[10px] leading-tight text-muted-foreground">{shape.label}</span>
              </button>
            ))}
          </div>
          <ColorField label="Shape color" value={shapeColor} onChange={setShapeColor} />
        </PopoverContent>
      </Popover>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {DRAW_MODES.map((item) => (
              <Button
                ...
                onClick={() => studio.setDrawMode(studio.drawMode === item.mode ? "off" : item.mode)}
              >
                {item.label}
              </Button>
            ))}
          </div>
          <ColorField label="Brush color" value={studio.drawColor} onChange={studio.setDrawColor} />
          <SliderField label="Brush size" value={studio.drawWidth} min={2} max={60} onChange={studio.setDrawWidth} />
          {studio.drawMode !== "off" && (
      ...
      <Popover>
        ...
        <PopoverContent side="right" align="start" className="w-64 space-y-2 p-3">
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
          ...
          />
          <Button type="button" className="w-full" disabled={photoBusy} onClick={() => fileInputRef.current?.click()}>
          ...
          <div className="flex flex-col gap-1">
            {OVERLAY_LABELS.map((overlay) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">{overlay.hint}</span>
              </button>
            ))}
          </div>
        </PopoverContent>
      ...
      <Popover>
        ...
        <PopoverContent side="right" align="start" className="w-64 p-3">
          <ColorField label="Canvas background" value={studio.backgroundColor} onChange={studio.setBackgroundColor} />
        </PopoverContent>
      </Popover>
