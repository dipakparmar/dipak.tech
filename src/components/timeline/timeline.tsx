...
export function Timeline({
  ...
  const onPointerMove = useCallback(
    (e: ReactPointerEvent) => {
      ...
      if (pinchRef.current && p.size >= 2) {
        ...
        zoomTo(
          pinchRef.current.px * (dist / pinchRef.current.dist),
          (a.x + b.x) / 2
        );
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      zoomRef.current(
        (pxRef.current || 1) * (e.deltaY < 0 ? 1.15 : 1 / 1.15),
        e.clientX
      );
    };
  ...
  useEffect(() => {
    ...
    const target = Math.max(
      0,
      Math.min(x - el.clientWidth / 2, el.scrollWidth - el.clientWidth)
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex">
        ...
        <div
          ref={scrollRef}
          className="scrollbar-hover relative flex-1 overflow-x-auto overflow-y-hidden"
        >
          <div
            ref={contentRef}
          ...
          >
            ...
            <div
              ...
              style={{ top: rulerHeight, height: lanesH }}
            >
              {ticks.map((t) => (
                <div
                  key={t}
                  className="absolute top-0 h-full border-l border-border/25"
                  style={{ left: t * pxPerSec }}
                />
              ))}
            </div>
            ...
            <div
              ...
              style={{ left: playheadX, height: timelineH }}
            >
