...
import { Timeline, TimelineTrack, formatTC } from '@/components/timeline/timeline'
...
function Waveform({ peaks, fill }: { peaks: number[]; fill: string }) {
  return (
    <svg viewBox={`0 0 ${PEAKS} 44`} preserveAspectRatio="none" className="h-full w-full">
      {peaks.map((p, i) => {
        const h = Math.max(1, p * 38)
        return <rect key={i} x={i} y={(44 - h) / 2} width={0.7} height={h} className={fill} />
      })}
    </svg>
  )
}

export function StemPlayer({ stems, filenameBase }: { stems: PlayerStem[]; filenameBase: string }) {
  const duration = stems[0]?.buffer.duration ?? 0
  const peaks = useMemo(() => stems.map((s) => computePeaks(s.buffer)), [stems])
        ...
        <div className="flex h-full items-center gap-1 px-2">
          <button
            ...
            className={`flex h-5 w-5 items-center justify-center rounded text-[10px] font-bold ${
              isMuted ? 'bg-red-500 text-white' : 'bg-muted text-muted-foreground hover:text-foreground'
            }`}
          >
            M
          </button>
          <button
            ...
            title="Solo"
            className={`flex h-5 w-5 items-center justify-center rounded text-[10px] font-bold ${
              isSolo ? 'bg-violet-500 text-white' : 'bg-muted text-muted-foreground hover:text-foreground'
            }`}
          >
            S
          </button>
          <span className="ml-0.5 truncate text-xs font-medium capitalize">{s.name}</span>
        </div>
      )
    }
  })
  ...
  const transport = (
    <>
      ...
      <span className="font-mono text-sm tabular-nums text-muted-foreground">/ {formatTC(duration)}</span>
    </>
  )
    ...
    <div>
      <Timeline
        ...
        renderLane={({ index, track }) => (
          <div className={`h-full w-full ${audible(track.id) ? '' : 'opacity-40'}`}>
            <Waveform peaks={peaks[index]} fill={STEM_FILL[track.id] ?? 'fill-violet-500'} />
          </div>
        )}
      />
      ...
      <p className="mt-2 text-center text-[11px] text-muted-foreground">
        ...
        <span className="font-bold">S</span> solo · <span className="font-bold">M</span> mute
      </p>
    </div>
  )
}
